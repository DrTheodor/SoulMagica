...
public class LightTreeStructure extends Structure<NoFeatureConfig> {
    public LightTreeStructure(Codec<NoFeatureConfig> codec) {
        super(codec);
    }
    ...
    public  IStartFactory<NoFeatureConfig> getStartFactory() {
        return LightTreeStructure.Start::new;
    }
...
//    }
    ...
    public static class Start extends StructureStart<NoFeatureConfig>  {
        ...
        public void func_230364_a_(DynamicRegistries dynamicRegistryManager, ChunkGenerator generator, TemplateManager templateManagerIn, int chunkX, int chunkZ, Biome biomeIn, NoFeatureConfig config) {
            ...
            LightTreePieces.start(templateManagerIn, blockpos, rotation, this.components, this.rand);

            // Sets the bounds of the structure.
            this.recalculateStructureSize();

            // I use to debug and quickly find out if the structure is spawning or not and where it is.
            //SoulMagica.LOGGER.log(Level.DEBUG, "Rundown House at " + (blockpos.getX()) + " " + blockpos.getY() + " " + (blockpos.getZ()));
            System.out.println("Light Tree at: " + (blockpos.getX()) + " " + blockpos.getY() + " " + (blockpos.getZ()));
        }
